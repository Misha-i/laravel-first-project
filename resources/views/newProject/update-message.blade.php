        ...
        <div class="col-md-7 col-lg-8">
            <h4 class="mb-3">{{ $car->car }}</h4>
                <div class="form-group">
                    ...
                    <input type="text" name="name" value="{{ $car->car }}" placeholder="Введіть" id="name" class="form-control">
                </div>
            <label for="car">Виберіть користувача</label>
            <select class="form-select" name="contact_id" aria-label="Default select example">
                @foreach( $contacts as $contact )
                    <option value="{{ $contact->id }}" id="car" @if($car->contact_id==$contact->id) selected @endif>{{ $contact->name }}</option>
                @endforeach
            </select>


              {{--  копія коду знизу це працюючий код--}}
            {{--<label for="car">Виберіть користувача</label>
            <select class="form-select" name="contact_id" aria-label="Default select example">
                    @foreach( $contacts as $contact )
                    <option value="{{ $contact->id }}" @if($data->user_id==$contact->id) selected @endif>{{ $contact->name }}</option>
                    @endforeach
                </select>--}}
